...
def preProcessing(img):
    grayscale = preGrayscale(img)
    cStretch = preContrastStreatching(grayscale)
    mFilter = preMedianFiltering(cStretch)

    # save file
    path_file = ('static/%s.jpg' % uuid.uuid4().hex)
    cv2.imwrite(path_file, mFilter)
    return json.dumps(path_file)

def GeometricActiveContour(img):
